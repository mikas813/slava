...
import {Aperture, Camera, Keyword, Lens, Location, Params, Shutter, Zoom} from '../../IconsSvg'
import {useDispatch, useSelector} from 'react-redux'
import {deleteImage, listImages, saveImage} from '../../redux/actions/imageActions'
...
const ManagePage = () => {
  ...
  const {loading: loadingSave, success: successSave, error: errorSave} = imageSave
  const imageDelete = useSelector( state => state.imageDelete )
  const {loading: loadingDelete, success: successDelete, error: errorDelete} = imageDelete
  ...
  const submitHandler = (e) => {
    ...
    dispatch( saveImage( {
      _id: id,
      image, keyword, location, camera,
      lens, zoom, aperture, shutter, iso
    } ) )
  }
      ...
      <ContentWrapper>
        ...
        <div>
          ...
          <div>
            <button
              onClick={() => openModal( {} )}
            >{modalVisible ? 'Edit all images' : 'Upload new image'}
            </button>
              ...
              : loading
                ...
                : error
                  ...
                  : <div className="edit-content_wrapper">
                    <div className="p-2">You have <span className="success">{images.length}</span> pictures in your gallery!</div>
                    {
                      images.map( (image, index) => (


                        <div key={image._id} className="edit-content_item">
                          ...
                          <a className="edit-image" onClick={() => openModal( image )}>. . .</a>
                          <a className="remove-image" onClick={() => deleteHandler( image )}/>
                        </div>
                      ) )
                    }
                  </div>
...

        {
          ...
          <div className="add-page_inputs">
            ...
            <form onSubmit={submitHandler}
              ...
              {errorSave && <div className="error">Invalid email or password</div>}
              ...
              <label htmlFor="keyword">Add keywords (max 3 words)</label>
              <input name="addKeywords" type="text"
                     value={keyword}
                     onChange={(e) => setKeyword( e.target.value )}
              />

              <label htmlFor="location">Add location (country)</label>
